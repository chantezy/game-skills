---
name: "game-dev-bridge"
description: "策划案到开发映射规范技能，定义配置表Schema、状态机模板、字段映射规则、网络同步策略和项目工程规范。"
trigger: "当用户需要将策划案转化为代码、设计配置表结构、定义状态机或确定网络同步策略时"
---

# 策划案到开发映射规范（Dev Bridge Spec）

## 概述

本文档是**策划案与程序开发之间的桥梁**。定义策划案中的设计描述如何转化为程序员（或 AI）可直接编码的技术规范，包括配置表 Schema、状态机模板、字段映射规则、网络同步策略和项目工程规范。

本文档解决的核心问题：策划案写了"移动速度 3 m/s，闪避无敌帧 0.3s"，程序员需要知道——这些参数放在哪个配置表？表结构是什么？客户端还是服务端校验？状态机怎么切？本文档给出标准化的答案。
