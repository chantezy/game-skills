...
description: "游戏技术实现全链路技能,覆盖策划案到代码的完整路径:从规范层(配置表Schema、状态机模板、字段映射、网络同步、工程规范)到执行层(技术选型、实现链路、代码生成、代码审查)"
trigger: "当用户需要将策划案转化为代码、设计配置表结构、定义状态机、确定网络同步策略、选择引擎架构、生成代码或进行代码审查时"
---
