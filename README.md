...
- `game-full-workflow` 作为框架文档构建器,负责模块索引和跨模块校验,不复制各模块内容
...
├── game-full-workflow/         ← GDD框架文档构建器+一致性校验器(模块索引+跨模块校验,不复制内容)
...
| game-full-workflow | 已有各模块策划案内容,需要构建GDD框架文档(模块索引表),或需要校验多模块策划案之间的一致性与前后逻辑时 |
...
各单模块技能(系统/关卡/战斗/剧情/角色/数值/美术/QA) → 产出各模块策划案
        ...
        │
        ▼
game-full-workflow(框架文档构建器+校验器) → 模块索引 + 前后逻辑校验 → GDD框架文档
```
...
5. 如需组装:get_skill("game-full-workflow")  → 获取 GDD 框架文档构建器工作流
```
